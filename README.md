...
### 실행 되었을때 빈칸이 다 채워져 있는지 확인할 수 있도록 checkValue 함수를 써서 유효성 검사를 하고 빈칸이 있다면 그것에 맞게 다시 입력하라는 
### 문구를 띄워준후에 빈칸으로 커서가 바뀌는 코드입니다. 
...
### 회원번호는 int형으로 받아주고 다른 6개는 string형으로 꽂아주게되면 pstmt.executeUpdate를 통해서 테이블에 추가가 됩니다.
